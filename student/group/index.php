...
$sql3 = "SELECT * FROM `groups` WHERE userid = '$userid'";
$result3 = $conn->query($sql3);

$sql4 = "SELECT * FROM `status` WHERE id = '1'";
$result4 = $conn->query($sql4);

$sql6 = "SELECT * FROM `file` WHERE userid = '$userid' ORDER BY `create_at` DESC";
$result6 = $conn->query($sql6);

$sql7 = "SELECT * FROM `file` WHERE userid = '$userid' ORDER BY `create_at` DESC";
      ...
      <div class="layout-page">
        ...
        <div class="content-wrapper">
          ...
          <div class="container flex-grow-1 container-p-y content-file">
            ...
            <p style="font-size: 16px;" class="badge rounded-pill bg-label-info ">ส่งงานกลุ่ม</p>

            <?php
            while ($row2 = $result3->fetch_assoc()) {
              $gid = $row2['id'];
              $sql5 = "SELECT * FROM `group_detal` JOIN users ON group_detal.userid = users.username  WHERE groupid = '$gid'";
              $result5 = $conn->query($sql5);
              ?>

              <h6 class="card-title">กลุ่ม:
                <?php echo $userid; ?>
              </h6>
              <h6 class="card-title">หัวข้อ:
                <?php echo $row2['topic']; ?>
              </h6>
              <h6 class="card-title">แอปพลิเคชัน:
                <?php echo $fname; ?>
              </h6>
            ...
            while ($row4 = $result4->fetch_assoc()) {
              ...
              if ($row4['statustype'] != "1") {
                $db_datetime2 = new DateTime($row4["datetime"]);
                if ($row4['type'] == "2") {
                  ?>
                  <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="row d-flex align-items-center">
                      <div class="col-md-8 d-flex align-items-center">
                        <input type="file" name="doc_file[]" id="fileToUpload" required class="form-control"
                          multiple="multiple"
                      ...
                      <div class="col-md-4 d-flex align-items-center">
                        ...
                        <div class="w-100 mx-2">
                          <div class="progress" style="display:none;" role="progressbar" aria-label="Animated striped example"
                  ...
                  $db_datetime = new DateTime($row4["datetime"]);
                  $current_datetime = new DateTime();
                  $current_datetime->setTimezone(new DateTimeZone('Asia/Bangkok'));

                  $interval = $db_datetime->diff($current_datetime);

                  // คำนวณว่ามีชั่วโมงเหลือเท่าไหร่หลังจากการลบชั่วโมงที่หลุดออกไปจากวันนั้น
                  $remaining_hours = $interval->days * 24 + $interval->h;
                  // echo $interval->format("ฃ %m เดือน %d วัน %h ชั่วโมง %i นาที %s วินาที");
            
                  $db_datetime = new DateTime($row4["datetime"], new DateTimeZone('Asia/Bangkok'));
                  $current_datetime = new DateTime('now', new DateTimeZone('Asia/Bangkok'));

                  // หาช่วงเวลาที่ต่างกันระหว่างวันที่
                  $interval = $db_datetime->diff($current_datetime);
                      ...
                      <tbody>
                        ...
                        if ($result->num_rows > 0) {
                          ...
                          while ($row = $result->fetch_assoc()) {
                            if ($row['delete_at'] == "") {
                              $file_size_MB = $row['filesize'] / (1024 * 1024);
                              ...
                              <tr>
                                ...
                                <td scope="row">
                                  ...
                                  if ($row['filetype'] == ".pdf") {
                                    ...
                                    ?>
                                          <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="#F15642"
                                    ...
                                    <?php
                                  } else if ($row['filetype'] == ".jpg" or $row['filetype'] == ".jpeg") {
                                    ?>
                                ...
                                <td><a href="/hci66/data/<?php echo $userid ?>/<?php echo $row['hash'] ?>" target="_blank">
                                    <?php echo $row['orginal'] ?>
                                  </a></td>
                                <td>
                                  <?php echo strtoupper($row['filetype']) ?>
                                </td>
                                <td>
                                  <?php
                                  if ($file_size_MB > 1) {
                          ...
                          <div id="content-blank">
                            <div class="text">
                              <h4>น้องๆยังไม่มีไฟล์งานเลยน้าา</h4>
                              <p>มาอัปโหลดไฟล์งานกันเถอะ!</p>
                            </div>
                          </div>
                          <?php
                        }
              ...
              <div class="modal-dialog modal-sm modal-dialog-centered modal-delete" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <form action="/hci66/student/group/" method="post">
...
<?php
            while ($rownew = $result7->fetch_assoc()) {
              ?>

              <div class="modal fade" id="new_<?php echo $rownew['id'] ?>" tabindex="-1" aria-labelledby="delete"
                aria-hidden="true">
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      คุณแน่ใจที่จะกู้คืนเอกสาร <b>
                        <?php echo $rownew['orginal'] ?>
                      </b> หรือไม่?
                    </div>
                    <div class="modal-footer">
                      <form action="/hci66/admin/group/upload.php?id=<?php echo $id ?>" method="post">
                        <input type="hidden" name="new" value="<?php echo $rownew['id'] ?>">
          ...
          <script src="api_group.js"></script>
